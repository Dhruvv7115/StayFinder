    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center flex-col p-4 gap-6">
      ...
      <div className="w-100 bg-white p-8 rounded-lg shadow-md flex flex-col items-center gap-8 border border-gray-200">
        ...
        <div className="relative flex items-center justify-center flex-col gap-2">
          <p className="text-sm font-medium text-gray-800">Upload a profile picture</p>
          {!avatar && 
          <Camera 
          onClick={handleIconClick} 
          className="absolute top-5/8 left-1/2 transform -translate-x-1/2 -translate-y-1/2"/>}
          {avatar && 
          <img 
          src={URL.createObjectURL(avatar)} 
          alt="avatar" 
          onClick={handleIconClick}
          className="w-20 h-20 rounded-full absolute top-5/8 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
          />}
          <input 
          type="file" 
          file={avatar}
          onChange={(e) => setAvatar(e.target.files[e.target.files.length - 1])}
          name="avatar" 
          accept="image/*" 
          ref={fileInputRef}
        ...
        <div className="flex flex-col items-center w-full">
          ...
          {error && <p className="text-sm text-red-500 mt-2 hidden">
            Error signing up. Please try again.
          </p>}
          <p>Already have an account? <Link to="/login" className="text-rose-500 hover:text-rose-600 underline">Login</Link></p>
        </div>
      </div>
    </div>
  )
